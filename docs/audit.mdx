...
This local-only path analyzes Cursor's exported usage rows instead of OpenClaw session logs.
...
- OpenClaw-only findings like retry waste and loop waste are intentionally disabled for this source
