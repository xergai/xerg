...
Remote and Railway audits can still use `--compare`, but the baseline lives in the local SQLite database on the machine running Xerg. Xerg pulls the remote telemetry to your machine and analyzes it locally. Re-run the same target with the same effective remote paths if you want clean before and after deltas.
