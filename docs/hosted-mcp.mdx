...
description: Connect Xerg's hosted audit history, trends, recommendations, and selected control actions to remote MCP clients.
---

Xerg offers a hosted MCP server for paid workspaces. It lets remote MCP clients query the hosted data already pushed into your workspace instead of reading OpenClaw logs directly from disk.

> Hosted MCP requires a Pro or Enterprise workspace plus a workspace API key.

## When to use hosted MCP

Use hosted MCP when you want a remote client such as Cursor or Claude Code to:
...
Use the local CLI when you need to read raw OpenClaw logs, run fresh audits from local, SSH, or Railway sources, or generate export files directly on your machine.
...
1. run a local audit with `xerg audit`
2. push it with `xerg audit --push`
3. confirm the hosted workspace has data
4. connect your MCP client to `https://mcp.xerg.ai/mcp`
