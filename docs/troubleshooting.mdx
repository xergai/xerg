...
| --- | --- | --- |
| `No OpenClaw sources were detected...` or exit code `2` | Xerg could not find any local or pulled OpenClaw data. | Run `npx @xerg/cli doctor`, then pass `--log-file` and `--sessions-dir` as needed, or inspect SSH or Railway sources directly. |
