...
Xerg runs its analysis locally. The CLI detects OpenClaw source files, normalizes them into runs and calls, computes spend, generates findings, and renders the report on your machine.

## Local audit flow

```text
OpenClaw gateway logs + session transcripts
  -> xerg audit
...
1. Detect OpenClaw source files from defaults or explicit paths.
...
- SSH
- Railway
- multi-source config via `--remote-config`
