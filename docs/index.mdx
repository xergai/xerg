---
title: Xerg
description: Start with local OpenClaw or Hermes audits, compare fixes, use remote OpenClaw audits when needed, and add hosted follow-up only if you want it.
---
...
2. Start with [`xerg init`](/init) or [run the quickstart](/quickstart).
3. Keep the most useful commands nearby with the [CLI cheat sheet](/cli-cheat-sheet).
4. If the data lives elsewhere, start with [doctor](/doctor). Use [remote audits](/remote-audits) for OpenClaw SSH and Railway targets.
5. Learn the core model in [waste taxonomy](/waste-taxonomy), [metrics](/metrics), and [how it works](/how-it-works).
6. When you want hosted features, use [connect and push](/auth-and-push) and [hosted MCP](/hosted-mcp).
7. When you want local automation, use [CI gates](/ci-gates).

## Hosted and Pro features

If you use Xerg Pro or Enterprise, you can:

- connect and push audit summaries into a hosted workspace with [authentication and push](/auth-and-push)
- configure Cursor or Claude Code for hosted audit access with [hosted MCP](/hosted-mcp)
