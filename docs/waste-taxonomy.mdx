...
description: Understand Xerg's finding kinds, confidence levels, and the difference between confirmed waste and directional opportunities.
---
...
For Cursor usage audits, Xerg flags extreme long-session cache carryover as waste when too much stale cached context is being dragged through the session.
...
- the Action queue in the terminal report may prioritize an opportunity even when it is not confirmed waste
